...
#undef TREEOP_FUNC_TEST_OPTIONAL
    ...
    static constexpr bool value =
        !std::is_reference_v<_Node> && !std::is_const_v<_Node> &&
            ...
            std::is_copy_assignable_v<_Holder> &&
            std::is_move_constructible_v<_Holder> &&
            !std::is_reference_v<_Key> && !std::is_const_v<_Key> &&
            std::is_copy_assignable_v<_Key> && has_getNthChild &&
            has_setNthChild && std::is_same_v<_Value, void> ||
        (has_setNthHolderValue ||
         (has_getNthHolderRef && has_setHolderValue)) &&
            ...
            has_getKey && has_keyCompareLess && has_nodeCompareEqual;
};

template <
    typename T, typename NODE, typename HOLDER, typename KEY, typename VALUE,
    std::enable_if_t<treeop_traits<T, NODE, HOLDER, KEY, VALUE>::value, bool> =
        true>
struct BTreeOpWrapper {
    ...
    inline NODE getFirstChild(NODE node) const {
        return m_ops.getNthChild(node, 0);
    }
    inline NODE getLastChild(NODE node) const {
        return m_ops.getNthChild(node, m_ops.getNumberOfChildren(node) - 1);
    }

    inline HOLDER getFirstHolder(NODE node) const {
        return m_ops.getNthHolder(node, 0);
    }
    inline HOLDER getLastHolder(NODE node) const {
        return m_ops.getNthHolder(node, m_ops.getNumberOfKeys(node) - 1);
    }

    inline KEY getFirstKey(NODE node) const { return m_ops.getNthKey(node, 0); }
    inline KEY getLastKey(NODE node) const {
        return m_ops.getNthKey(node, m_ops.getNumberOfKeys(node) - 1);
    }
    ...
    using HolderValue =
        std::conditional_t<std::is_same_v<VALUE, void>, dummy_struct, VALUE>;
    ...
    inline void setNthHolderValue(NODE node, size_t nth, HolderValue val) {
        if constexpr (traits::has_setNthHolderValue) {
            ...
            static_assert(
                std::is_same_v<VALUE, void> || traits::has_setNthHolderValue ||
    ...
    inline size_t getNumberOfKeys(NODE node) const {
        return m_ops.getNumberOfKeys(node);
    }
    ...
    inline bool isFull(NODE node) const {
        return this->getNumberOfKeys(node) == 2 * this->getOrder() - 1;
    }
    ...
    inline bool isEmptyNode(NODE node) const {
        return !m_ops.isNullNode(node) &&
               m_ops.getNumberOfChildren(node) == 0 &&
               m_ops.getNumberOfKeys(node) == 0;
    }
...
template <
    typename T, typename NODE, typename HOLDER, typename KEY, typename VALUE,
    bool enableParent = true, bool multikey = false,
    size_t static_vector_size = 32,
    std::enable_if_t<treeop_traits<T, NODE, HOLDER, KEY, VALUE, true>::value,
                     bool> = true>
...
public:
    ...
    using NodePath = std::conditional_t<
        ...
        std::conditional_t<
            ...
            maxsize_vector<std::pair<NODE, size_t>, static_vector_size>,
...
private:
    template <typename N,
              std::enable_if_t<std::is_same_v<N, NODE>, bool> = true>
    ...
    inline std::pair<KEY, NODE> splitFullNode(NODE& root, NODE parent,
                                              size_t nodeIdx, NODE node) {
...
public:
    ...
    inline void check_consistency(NODE root) const {
        ...
        for (; !queue.empty(); queue.pop()) {
            ...
            if (m_ops.isLeaf(node)) {
                ...
                assert(m_ops.getNumberOfKeys(node) + 1 ==
    ...
    inline HolderPath insertHolder(NODE& root, HOLDER&& holder) {
        ...
        while (true) {
            ...
            if constexpr (!multikey) {
                if (currentIdx > 0 &&
                    m_ops.keyCompareEqual(
                        key, m_ops.getNthKey(current, currentIdx - 1))) {
    ...
    inline HOLDER deleteHolder(NODE& root, HolderPath path) {
                ...
                if (idx > 0) {
                    ...
                    if (m_ops.getNumberOfKeys(sibling_prev) > t - 1) {
                        ...
                        const auto sib_prev_n =
                            m_ops.getNumberOfKeys(sibling_prev);
                        auto ph = m_ops.extractNthHolder(sibling_prev,
                                                         sib_prev_n - 1);
                    ...
                    if (m_ops.getNumberOfKeys(sibling_next) > t - 1) {
                        const auto sib_next_n =
                            m_ops.getNumberOfKeys(sibling_next);
        ...
        const bool shiftLeft =
            path.m_index + 1 < m_ops.getNumberOfKeys(leafNode);
    ...
    inline HolderPath findKey(NODE root, const KEY& key) {
        ...
        for (auto node = root; !m_ops.isNullNode(node);) {
            ...
            if (kn > 0 &&
                m_ops.keyCompareEqual(key, m_ops.getNthKey(node, kn - 1))) {
    ...
    inline void forward(NODE root, HolderPath& path) {
        ...
        if (m_ops.isLeaf(node)) {
            if (path.m_index + 1 < m_ops.getNumberOfKeys(node)) {
                ...
                path.m_index = u;
                while (!m_ops.isNullNode(p) &&
                       u + 1 == m_ops.getNumberOfChildren(p)) {
            ...
            for (auto n = m_ops.getNthChild(node, index);;
                 n = m_ops.getFirstChild(n)) {
                this->NodePathPush<NodePath>(path.m_path, n, index);
                index = 0;
                if (m_ops.isLeaf(n)) break;
            }
            path.m_index = 0;
        }
    }
    ...
    template <
        typename U,
        std::enable_if_t<std::is_same_v<HOLDER, decltype(std::declval<U>()())>,
                         bool> = true>
    inline NODE initWithAscSequence(size_t size, U iterFunc) {
        ...
        const std::function<NODE(size_t, size_t)> initNode = [&](size_t d,
                                                                 size_t s) {
...
template <typename _Key, typename _Value, typename _CmpLess,
          typename _Allocator, size_t Order, bool parentsOps>
struct TreeNodeOps {
    ...
    inline size_t getNumberOfKeys(_Node node) const {
        return node->m_holders.size();
    }
...
template <typename _Key, typename _Value, typename _CmpLess,
          typename _Allocator, size_t Order, bool parentsOps, bool multikey>
using BTreeAlgo = BTreeAlgorithmImpl::BTreeAlgorithm<
    ...
    typename TreeNodeOps<_Key, _Value, _CmpLess, _Allocator, Order,
                         parentsOps>::KVPair,
    _Key, _Value, parentsOps, multikey>;

template <typename _Key, typename _Value, typename _CmpLess,
          typename _Allocator, size_t _Order, bool _parentsOps, bool _multikey>
struct BTreeInMemory : protected BTreeAlgo<_Key, _Value, _CmpLess, _Allocator,
                                           _Order, _parentsOps, _multikey> {
    using BASE = BTreeAlgo<_Key, _Value, _CmpLess, _Allocator, _Order,
                           _parentsOps, _multikey>;
    using treeops_t =
        TreeNodeOps<_Key, _Value, _CmpLess, _Allocator, _Order, _parentsOps>;
