...
#undef TREEOP_FUNC_TEST_OPTIONAL
    ...
    static constexpr bool value =
        ...
        std::is_copy_assignable_v<_Node> && !std::is_reference_v<_Key> &&
        !std::is_const_v<_Key> && has_getLeft && has_setLeft && has_getRight &&
        has_setRight && has_isBlack && has_setBlack && has_isNullNode &&
        has_getNullNode && has_getKey && has_keyCompareLess &&
...
struct RbtreeOpWrapper {
    ...
    inline bool isBlackOrNULL(NODE node) const {
        return m_ops.isNullNode(node) || m_ops.isBlack(node);
    }
    ...
    inline bool keyCompareLess(NODE lhs, NODE rhs) const {
        return m_ops.keyCompareLess(m_ops.getKey(lhs), m_ops.getKey(rhs));
    }
...
template <typename T, typename NODE, typename KEY, bool multikey = false,
          bool ParentsOps = true, size_t static_vector_size = 128,
          std::enable_if_t<treeop_traits<T, NODE, KEY, true>::value, bool> =
              true>
class RBTreeAlgorithm {
public:
    using traits = treeop_traits<T, NODE, KEY>;
    static constexpr bool parents_ops =
        traits::has_getParent && traits::has_setParent && ParentsOps;
    using NodePath = std::conditional_t<
        parents_ops, NODE,
        std::conditional_t<static_cast<bool>(static_vector_size > 0),
                           maxsize_vector<NODE, static_vector_size>,
                           std::vector<NODE>>>;
...
private:
    ...
    inline void SwapAncestorAndDescendant(N n1, N n2) {
        ...
        if (m_ops.nodeCompareEqual(m_ops.getParent(n2), n1)) {
            ...
            const bool is_left_child =
                m_ops.nodeCompareEqual(m_ops.getLeft(n1), n2);
            const bool n1_is_left_child =
                !m_ops.isNullNode(p)
                    ? m_ops.nodeCompareEqual(m_ops.getLeft(p), n1)
                    : false;
            const auto n1_left = m_ops.getLeft(n1),
                       n1_right = m_ops.getRight(n1);
            const auto n2_left = m_ops.getLeft(n2),
                       n2_right = m_ops.getRight(n2);
            ...
            const bool n1_is_left =
                !m_ops.isNullNode(n1_parent)
                    ? m_ops.nodeCompareEqual(m_ops.getLeft(n1_parent), n1)
                    : false;
            const auto n1_left = m_ops.getLeft(n1),
                       n1_right = m_ops.getRight(n1);

            const auto n2_parent = m_ops.getParent(n2);
            const bool n2_is_left =
                !m_ops.isNullNode(n2_parent)
                    ? m_ops.nodeCompareEqual(m_ops.getLeft(n2_parent), n2)
                    : false;
            const auto n2_left = m_ops.getLeft(n2),
                       n2_right = m_ops.getRight(n2);
    ...
    inline void SwapAncestorAndDescendant(N& ance, N& desc) {
        ...
        if (ance.size() + 1 == desc.size()) {
            const bool desc_is_left =
                m_ops.nodeCompareEqual(m_ops.getLeft(ance.back()), desc.back());
            assert(m_ops.nodeCompareEqual(m_ops.getLeft(ance.back()),
                                          desc.back()) ||
                   m_ops.nodeCompareEqual(m_ops.getRight(ance.back()),
                                          desc.back()));
            ...
            if (ance.size() > 1) {
                ...
                if (m_ops.nodeCompareEqual(m_ops.getLeft(pp), ance.back())) {
                    ...
                    assert(m_ops.nodeCompareEqual(m_ops.getRight(pp),
                                                  ance.back()));
            ...
            if (desc_is_left) {
                ...
                assert(m_ops.nodeCompareEqual(m_ops.getRight(ance.back()),
                                              desc.back()));
            ...
            const auto n1_parent =
                ance.size() > 1 ? ance[ance.size() - 2] : m_ops.getNullNode();
            ...
            if (!m_ops.isNullNode(n1_parent)) {
                if (m_ops.nodeCompareEqual(m_ops.getLeft(n1_parent),
                                           ance.back())) {
                    m_ops.setLeft(n1_parent, desc.back());
                } else {
                    assert(m_ops.nodeCompareEqual(m_ops.getRight(n1_parent),
                                                  ance.back()));
            ...
            if (m_ops.nodeCompareEqual(m_ops.getLeft(n2_parent), desc.back())) {
                ...
                assert(m_ops.nodeCompareEqual(m_ops.getRight(n2_parent),
                                              desc.back()));
...
public:
    ...
    inline NodePath insertNode(NODE& root, NODE node) {
        ...
        for (;;) {
            if constexpr (!multikey) {
                if (m_ops.keyCompareEqual(m_ops.getKey(node),
                                          m_ops.getKey(cn))) {
                    return ans;
                }
            }
        ...
        for (auto p = this->GetNodeAncestor(nodepath, 1);
            ...
            if (m_ops.nodeCompareEqual(this->GetNodeAncestor(nodepath, 1),
                                       m_ops.getNullNode())) {
                root = node;
                m_ops.setBlack(node, true);
            }
    ...
    void check_consistency(NODE root) const {
        ...
        for (; !queue.empty(); queue.pop()) {
            ...
            if (m_ops.isNullNode(m_ops.getLeft(node)) ||
                m_ops.isNullNode(m_ops.getRight(node))) {
                is_end = true;
            }
    ...
    inline NODE deleteNode(NODE& root, NodePath nodePath) {
        ...
        bool need_extra_black =
            m_ops.isBlack(node) && !m_ops.isNullNode(node_parent);
        if (m_ops.isNullNode(m_ops.getLeft(node))) {
            ...
            if (!m_ops.isNullNode(node_parent)) {
                if (m_ops.nodeCompareEqual(m_ops.getLeft(node_parent), node)) {
                    ...
                    assert(m_ops.nodeCompareEqual(m_ops.getRight(node_parent),
                                                  node));
            ...
            extra_black =
                need_extra_black ? m_ops.getRight(node) : m_ops.getNullNode();
        } else {
        ...
        for (; m_ops.isBlackOrNULL(extra_black) &&
            ...
            if (extra_is_left_child) {
                ...
                if (!m_ops.isBlack(sibling)) {
                    ...
                    if (m_ops.isNullNode(
                            this->GetNodeAncestor(extra_parent_path, 1))) {
                    ...
                    const auto siblingx =
                        this->GetNodeAncestor(extra_parent_path, 0);
                ...
                if (m_ops.isBlackOrNULL(m_ops.getLeft(sibling)) &&
                    m_ops.isBlackOrNULL(m_ops.getRight(sibling))) {
                    m_ops.setBlack(sibling, false);
                    extra_black = extra_parent;
                    extra_parent = this->GetNodeAncestor(extra_parent_path, 1);
                    this->NodePathPop(extra_parent_path);
                    extra_is_left_child =
                        !m_ops.isNullNode(extra_parent)
                            ? m_ops.nodeCompareEqual(
                                  m_ops.getLeft(extra_parent), extra_black)
                            : false;
                    ...
                    bool pp_left =
                        !m_ops.isNullNode(pp) &&
                        m_ops.nodeCompareEqual(m_ops.getLeft(pp), extra_parent);
                ...
                assert(m_ops.isBlack(sibling));
                if (m_ops.isBlackOrNULL(m_ops.getRight(sibling)) &&
                    m_ops.isBlackOrNULL(m_ops.getLeft(sibling))) {
                    m_ops.setBlack(sibling, false);
                    extra_black = extra_parent;
                    extra_parent = this->GetNodeAncestor(extra_parent_path, 1);
                    this->NodePathPop(extra_parent_path);
                    ...
                    const auto pp = this->GetNodeAncestor(extra_parent_path, 1);
                    bool pp_right = !m_ops.isNullNode(pp) &&
                                    m_ops.nodeCompareEqual(m_ops.getRight(pp),
                                                           extra_parent);
                    ...
                    assert(!m_ops.isBlackOrNULL(m_ops.getLeft(sibling)));
                    auto sr = m_ops.getRight(sibling),
                         sl = m_ops.getLeft(sibling);
        ...
        assert(!m_ops.isBlack(extra_black) ||
               m_ops.nodeCompareEqual(extra_black, root));
    ...
    inline NodePath findNode(NODE root, KEY key) const {
        ...
        for (; !m_ops.isNullNode(cn);) {
            ...
            if (m_ops.keyCompareEqual(key, m_ops.getKey(cn))) return ans;
    ...
    inline NodePath end(NODE root) { return this->InitPath<NodePath>(); }
    ...
    inline void forward(NODE, NodePath& path) const {
        ...
        if (!m_ops.isNullNode(m_ops.getRight(node))) {
            for (node = m_ops.getRight(node); !m_ops.isNullNode(node);
                 node = m_ops.getLeft(node)) {
                this->NodePathPush(path, node);
            }
        } else {
            for (; !m_ops.isNullNode(node);
                 node = this->GetNodeAncestor(path, 0)) {
                if (!m_ops.isNullNode(this->GetNodeAncestor(path, 1)) &&
                    m_ops.nodeCompareEqual(
                        m_ops.getLeft(this->GetNodeAncestor(path, 1)), node)) {
    ...
    inline void backward(NODE root, NodePath& path) const {
        ...
        auto node = this->GetNodeAncestor(path, 0);
        if (!m_ops.isNullNode(m_ops.getLeft(node))) {
            for (node = m_ops.getLeft(node); !m_ops.isNullNode(node);
                 node = m_ops.getRight(node)) {
                this->NodePathPush(path, node);
            }
        } else {
            for (; !m_ops.isNullNode(node);
                ...
                if (!m_ops.isNullNode(this->GetNodeAncestor(path, 1)) &&
                    m_ops.nodeCompareEqual(
                        m_ops.getRight(this->GetNodeAncestor(path, 1)), node)) {
                    this->NodePathPop(path);
                    break;
                }
    ...
    template <
        typename U,
        std::enable_if_t<std::is_same_v<NODE, decltype(std::declval<U&>()())>,
                         bool> = true>
...
template <typename _Key, typename _Value, typename _CmpLess,
          typename _Allocator, bool parentsOps>
struct TreeNodeOps {
...
template <typename _Key, typename _Value, typename _CmpLess,
          typename _Allocator, bool parentsOps, bool multikey>
using RBTreeAlgo = RBTreeAlgorithmImpl::RBTreeAlgorithm<
    TreeNodeOps<_Key, _Value, _CmpLess, _Allocator, parentsOps>,
    TreeNode<_Key, _Value, parentsOps>*, _Key, multikey, parentsOps>;

template <typename _Key, typename _Value, typename _CmpLess,
          typename _Allocator, bool _parentsOps, bool _multikey>
struct RBTreeInMemory : protected RBTreeAlgo<_Key, _Value, _CmpLess, _Allocator,
                                             _parentsOps, _multikey> {
    using BASE =
        RBTreeAlgo<_Key, _Value, _CmpLess, _Allocator, _parentsOps, _multikey>;
    using treeops_t =
        TreeNodeOps<_Key, _Value, _CmpLess, _Allocator, _parentsOps>;
...
public:
    ...
    inline RBTreeInMemory(const RBTreeInMemory& _oth)
        : BASE(treeops_t(_oth.m_ops.ops().m_cmp)),
          m_allocator(_oth.m_allocator),
          m_root(nullptr),
          m_size(0) {
        *this = _oth;
    }
