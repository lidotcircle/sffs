...
template <typename T, typename = void>
struct has_std_to_string_2 : std::false_type {};

template <typename T>
struct has_std_to_string_2<
    T, std::void_t<decltype(std::declval<T>().to_string())> > : std::true_type {
};
...
template <typename T>
constexpr bool has_std_to_string_2_v = has_std_to_string_2<T>::value;
...
struct utest_type_deducer final {
    static void _(const T t) {
        ...
        } else if constexpr (has_std_to_string_v<T>) {
            const std::string s = std::to_string(t);
            UTEST_PRINTF("%s", s.c_str());
        } else if constexpr (has_std_to_string_2_v<T>) {
            const std::string s = t.to_string();
            UTEST_PRINTF("%s", s.c_str());
        } else {
            const auto n = sizeof(T);
            auto cptr = static_cast<const char*>(static_cast<const void*>(&t));
            const std::string s(cptr, cptr + n);
            UTEST_PRINTF("%s", to_hex_string(s).c_str());
        }
    }
};
...
static void test_debug_break() { __asm__ volatile("int $0x03"); }
#else
#define test_debug_break()
#endif
#endif

#ifdef __has_builtin
#if __has_builtin(__builtin_debugtrap)
#undef test_debug_break
#define test_debug_break() __builtin_debugtrap()
#endif
#endif
...
static int _g_utest_result = UTEST_TEST_PASSED;
static int* utest_result = &_g_utest_result;
...
int utest_main(int argc, const char* const argv[]) {
    ...
    for (index = 1; index < UTEST_CAST(size_t, argc); index++) {
        ...
        if (0 == UTEST_STRNCMP(argv[index], help_str, strlen(help_str))) {
            ...
            printf(
                "  --output=<output>       Output an xunit XML file to the "
                "file "
                "specified in <output>.\n"
    ...
    for (index = 0; index < utest_state.tests_length; index++) {
        _g_utest_result = UTEST_TEST_PASSED;
        ...
        if (UTEST_TEST_FAILURE == result ||
            UTEST_TEST_FAILURE == _g_utest_result) {
